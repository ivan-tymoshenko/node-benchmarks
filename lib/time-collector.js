...
class TimeCollector {
  constructor(
    ...
    this.intermediateValue.add = (...argv) => {
      if (!Math.round(Math.random() * this.bufferSize)) {
        this.intermediateValue.push(argv);
      }
    };
  }
  ...
  findAnomaly(
    ...
    const sumOfSquares = testBuffer.reduce((sum, time) => {
      const res = Math.pow((time - averageTime), 2) / bufferSize;
      return res + sum;
    }, 0);
    const delta = (Math.sqrt(sumOfSquares) * 100) / Math.sqrt(bufferSize);
    let totalTime = 0;
    let anomalyTime = 0;
    let anomalyCount = 0;
    testBuffer.forEach(time => {
      if (time - averageTime > delta) {
        anomalyCount++;
        anomalyTime += time;
      } else {
        totalTime += time;
  ...
  getTimeSyncFunc(
    ...
    for (let i = 0; i < this.count; i++) {
      ...
      fnEnvironment.add(end[0] * 1e9 + end[1]);
      this.intermediateValue.add(result, i);
    }
  ...
  getTimeAsyncFunc(
    ...
    const testFunction = (begin, i) => {
      fn((err, result) => {
        const end = process.hrtime(begin);
        this.intermediateValue.add(err, result, i);
        fnEnvironment.add(end[0] * 1e9 + end[1]);
        if (++i < this.count) testFunction(process.hrtime(), i);
        else {
  ...
  makeResults(
    ...
    const results = {
      ...
      anomaly: {
        time: getPercent(fnEnvironment.anomalyTime, fnEnvironment.totalTime),
        percent: getPercent(fnEnvironment.anomalyCount, this.count, 2)
      }
    };
